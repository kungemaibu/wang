...
public class WareHouseDaoImpl implements WareHouseDao {
	...
	public Pagerr findWareHouse(int curPage, int pageSize) {
		...
		List<Warehouse> list=this.hibernateTemplate.executeFind(new PageHibernateCallBackP<Warehouse>(hql, null, pageSize, curPage));
		Pagerr pa=new Pagerr(curPage,pageSize);
		pa.setList(list);
		List<Long> list1=this.hibernateTemplate.find("select count(*) from Warehouse");
		pa.setTotalRecords(Integer.valueOf(list1.get(0).toString()));
		return pa;
	}
